...
class PageUtils:
    ...
    def wait_visible(self, locator, timeout=5):
        ...
        while i != timeout:
            try:
                ...
                sleep(1)
                i += 1
        #print('Element never became visible: %s (%s)' % (locator[0], locator[1]))
    ...
    def get_text(self, locator):
        element = self.wait_visible(locator)
        # print('gettext pageutils')
        # print(element)
        # print('gettext pageutils text')
        # print(element.text)
        return element.text

    # get full locaator
    def get_locator(self, obj, locator_name):
        if platform == 'ios':
            return obj.get('platform-ios').get('class-authorisation').get(locator_name)
        else:
            return obj.get('platform-android').get('class-authorisation').get(locator_name)